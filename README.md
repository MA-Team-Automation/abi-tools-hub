...
- **主页**：居中大标题 + 5 张工具卡片（仅图标、名称与一句话描述），点击整卡进入对应工具页。右上角圆形图标按钮「检查更新」：Hub 自更新与 5 个工具共 6 个单元并行执行拉取代码、按需同步环境（仅拉到新提交或环境缺失时才跑 uv sync）；后台工作进行时标题下方显示真实进度（「正在更新 · N/6」+ 当前更新项 + 已耗时），完成后提示「一切就绪」。
...
| POST | `/api/refresh-all` | 后台并行全量更新（Hub 自更新 + 5 个工具同池并行，按需同步环境） |
...
启动 Web 控制台（或点击主页「检查更新」）时，Hub 会把**它自己**的自更新与 5 个工具的更新放进同一个并行池（共 6 个单元）同时执行：
...
- 环境同步按需执行：仅当某工具拉到了新提交或其 `.venv` 缺失时才运行 `uv sync`，否则记一行「环境未变化，跳过依赖同步」。
